...
**当资源不够用的时候需要虚拟化**，让资源可以被无限抽象（例如货币）
...
从程序视角（虚拟化技术），以stack为例，认为stack可以无限增长，永远不会碰到 heap

实际上stack被分为若干段，每个段有一个段描述符，指向page table中的条目



![image-20210628180305198](OS.assets/image-20210628180305198.png)	

*在计算机的世界中是不断地虚拟，让你不知道真实的是什么样的，一层层地抽象下去。*
...
<img src="OS.assets/image-20210628175650345.png" alt="image-20210628175650345" style="zoom:50%;" />	
...
* 在段上抽象出 block，固定大小 4KB，为每个对象分配内存，如对象需要 7KB，则分配两个block，并排序成连续的。
* 用链表维护 block，用链表和指针维护block的分配、回收、重排。
...
* 将block 按不同大小进行划分空间
* 每个block维护一个与之对应大小的链表

<img src="OS.assets/image-20210628193018644.png" alt="image-20210628193018644" style="zoom:50%;" />	



==思考==：

* 基于链表的抽象，要提前分配很多内存，有没有性能更高、内存占用更少的做法？
* 如何和页（page）产生更多的联系，比如回收都在一个页的内存？而非跨页，跨页会有一个中断请求





方案三：主流做法 cache - slot 方法

<img src="OS.assets/image-20210628193907481.png" alt="image-20210628193907481" style="zoom:50%;" />	
...
没有虚拟机的语言，如C，就直接用操作系统的机制来管理内存

有虚拟机的语言，如 V8、Java，由自己管理内存

*没有一个方法是最优解，只有适合自己的才是最优的*



<img src="OS.assets/image-20210628194832514.png" alt="image-20210628194832514" style="zoom:50%;" />	



通用问题：在多次分配、销毁之后，内存会产生很多碎片，需要整理内存，以容纳更多的东西

==难点==：专有线程在整理内存回收的时候，以 b 为例

* 专有线程需要挪b的位置
* 但又有进程要读写b的内容
* 两个线程会产生竞争，如果要完成整理，可能要应用进程停下来
...
1. 新分配的对象放到 eden （来自 Java）中

2. 如果发生销毁，那么将不需要销毁的内容拷贝到 survivors 中



<img src="OS.assets/image-20210628201915277.png" alt="image-20210628201915277" style="zoom:50%;" />	



<img src="OS.assets/image-20210628201941148.png" alt="image-20210628201941148" style="zoom:50%;" />	
